...
【保存・読み込み】
- 自動保存（オートセーブ）: 編集内容はブラウザのローカルストレージに自動で保存されます。ページを閉じても次回アクセス時に「前回の作業を復元しますか？」と表示され、復元できます
- 保存ボタン▾: クラウド保存（マイLPに保管）またはローカル保存（JSONファイル）を選択できます
- クラウド保存: ログイン中のみ利用可能。マイLPに保管され、どのデバイスからでもアクセスできます
...
- 箇条書きは「・」を使う
- 解決できない・載っていない質問には「このチャットで解決できない場合は info@ailpstudio.com までメールでお問い合わせください」と案内する
- 3文以内で答えられる場合は簡潔に
`;
